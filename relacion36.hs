...
grafoCiclo :: Int -> Grafo Int Int
grafoCiclo n = creaGrafo ND (1,n)
               ((n,1,0):[(x,x+1,0) | x <- [1..n-1]])
...
contiguos :: (Ix v,Num p) => Grafo v p -> v -> [v]
contiguos g v = nub $ buscaContiguos v (aristas g)
  where buscaContiguos :: (Ix v, Num p) => v -> [(v,v,p)] -> [v]
        buscaContiguos v ((x,y,_):xs)
            | v == x = y: buscaContiguos v xs
            | v == y = x: buscaContiguos v xs
            | otherwise = buscaContiguos v xs
        buscaContiguos _ _ = []
...
prop_GradoCompleto :: Int -> Bool
prop_GradoCompleto n = all (\x -> grado g x == n-1) (nodos g)
  where g = completo n
...
prop_completoRegular :: Int -> Bool
prop_completoRegular n = regularidad (completo n) == Just (n-1)
...
prop_cicloRegular :: Int -> Bool
prop_cicloRegular n = regularidad (grafoCiclo n) == Just 2
